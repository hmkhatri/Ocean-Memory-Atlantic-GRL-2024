"""
The script computes annual-mean temperature anomalies in the upper 200 m using EN4 dataset and SST using HadSST data.
Temperature anomalies are computed using composite analysis based on North Atlantic Oscillation (NAO) indices.
Temperatuer anomalies are computed at different lead/lag years in response to onset of a single NAO event.
Bootstrapping is then used to compute the mean and confidence intervals for annual-mean anomaly maps. 
"""
...
dz = ds_EN4['dz'] * ds_EN4['temperature'].isel(time=0) / ds_EN4['temperature'].isel(time=0) # get 3D dz to get only wet ocean grid cells
...
for nao1 in nao_var:
    ...
    # create composites (based on winter-mean NAO index extreme values that lie outside nao_cut_coef)
